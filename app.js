...
app.get("/user/following", authenticateToken, async (request, response) => {
  ...
  const query = `SELECT name FROM 
                  user INNER JOIN following ON user.user_id = follower.following_user_id
                  WHERE follower.follower_user_id = ${user_id};`;
  const followersArray = await db.all(query);
...
app.get("/tweets/:tweetId", authenticateToken, async (request, response) => {
  ...
  const tweetResult = await db.get(query);
  //response.send(tweetResult);
  const userFollowerQuery = `SELECT * FROM 
...
app.get(
  ...
  async (request, response) => {
    ...
    const { payload } = request;
    const { user_id, name, username, gender } = payload;
    const getLikedUsersQuery = `SELECT * FROM follower INNER JOIN tweet ON tweet.user_id = follower.following_user_id INNER JOIN like ON like.tweet_id = tweet.tweet_id
